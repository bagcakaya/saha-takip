...
export const CompanyService = {
  ...
  async fetchCompanies(): Promise<Company[]> {
    ...
    // 1. If in Local Server Mode
    if (MobileServerConfigService.isLocalMode()) {
      try {
        const res = await MobileServerConfigService.apiGet<{ id: number; tasks: string[]; notFound?: boolean }>(
        ...
        console.warn('Yerel sunucudan mobil kurum listesi çekilemedi:', err);
      }
      return local;
    }
  ...
  async syncToCloud(companies: Company[]): Promise<void> {
    ...
    if (MobileServerConfigService.isLocalMode()) {
      try {
        await MobileServerConfigService.apiPost(`/api/standard_tasks/${COMPANIES_SLOT_ID}`, {
          tasks: chunks,
        });
      } catch (err) {
        console.warn('Yerel sunucuya mobil kurum listesi kaydedilemedi:', err);
      }
      return;
    }
