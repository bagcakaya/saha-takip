...
export type ApprovalStatus = 'pending' | 'pending_approval' | 'approved' | 'rejected' | 'completed';
...
export interface HeadquarterCompany {
  id: string;
  companyCode: string;
  name: string;               // e.g. "POLATLAR A.Ş. (Merkez)", "Defacto", "LC Waikiki"
  taxNumber?: string;
  phone?: string;
  address?: string;
...
export interface Branch {
  id: string;
  companyCode: string;
  headquarterId?: string;     // Bağlı olduğu Merkez Firma ID'si
  headquarterName?: string;   // Bağlı olduğu Merkez Firma Adı
