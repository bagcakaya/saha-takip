...
export const ServicesView: React.FC = () => {
  ...
  return (
    <div className="space-y-5 pb-12 animate-in fade-in duration-300 w-full max-w-full overflow-hidden">
      {/* Top Toolbar: Search, Count & Add Button */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-3.5 sm:p-4 rounded-3xl shadow-xs w-full max-w-full overflow-hidden space-y-3">
        ...
        {/* Sub-bar: Count */}
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs font-semibold text-slate-500 dark:text-slate-400">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-orange-50 dark:bg-orange-950/40 text-orange-700 dark:text-orange-400 font-bold border border-orange-200/60 dark:border-orange-800/40">
              {searchQuery
                ? `${filteredServices.length} / ${services.length} Servis Kaydı`
                : `Toplam ${services.length} Servis Kaydı`}
            </span>
          </div>
          <span className="text-[11px] text-slate-400 hidden sm:inline">
            ℹ️ Servis kayıtları bilgilendirme amaçlıdır.
          </span>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl text-center space-y-3 shadow-xs">
          ...
          <div className="space-y-1">
            <h4 className="text-sm sm:text-base font-black text-slate-800 dark:text-slate-100">
              {searchQuery ? 'Arama Sonucu Bulunamadı' : 'Henüz Servis Kaydı Yok'}
            </h4>
