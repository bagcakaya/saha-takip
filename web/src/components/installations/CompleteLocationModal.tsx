...
import { X, CheckCircle2, MapPin, Send, Camera, Image as ImageIcon, Loader2, CheckSquare } from 'lucide-react';
import { LocationItem } from '../../types/storage';
import { compressImage } from '../../utils/imageUtils';

interface CompleteLocationModalProps {
  isOpen: boolean;
...
export const CompleteLocationModal: React.FC<CompleteLocationModalProps> = ({
  isOpen,
  location,
  onClose,
  onConfirm,
  ...
  useEffect(() => {
    if (isOpen) {
      setCompletionNote(location?.completionNote || '');
      setCompletionPhotos(location?.completionPhotos || []);
  ...
  const completedTasks = location.tasks.filter(
    (t) => t.status === 'completed' || t.status === 'not_present'
  ).length;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 dark:text-slate-100">
                {location.status === 'rejected' ? 'Eksikleri Gider / Tekrar Tamamla' : 'Kurulumu Tamamla'}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Kurulum tamamlama notunuz ve fotoğraflarınız yönetici onayına iletilecektir.
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800 space-y-1.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <span>Kurulum Yeri</span>
              </div>
              <div className="flex items-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400">
                <CheckSquare className="w-3.5 h-3.5" />
                <span>{completedTasks} / {totalTasks} Görev Tamam</span>
              </div>
            </div>
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
              Tamamlama Açıklaması / Teslim Notu
            </label>
            <textarea
              value={completionNote}
              onChange={(e) => setCompletionNote(e.target.value)}
              placeholder="Örn: Tüm checklist maddeleri eksiksiz uygulandı, hat testleri yapıldı ve firma yetkilisine teslim edildi..."
              rows={3}
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
            ...
            >
              ...
              <span>{isSubmitting ? 'İletiliyor...' : 'Yönetici Onayına Gönder'}</span>
            </button>
          </div>
        </form>
      </div>
