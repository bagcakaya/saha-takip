...
import { X, CheckCircle2, Wrench, Send, Camera, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export const CompleteServiceModal: React.FC<CompleteServiceModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      setCompletionNote(service?.completionNote || '');
      setCompletionPhotos(service?.completionPhotos || []);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 dark:text-slate-100">
                {service.status === 'rejected' ? 'Eksikleri Gider / Tekrar Tamamla' : 'Servisi Tamamla'}
              </h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800 space-y-1.5">
            ...
            <p className="text-xs text-slate-700 dark:text-slate-300 line-clamp-2">
              <span className="font-semibold text-slate-500">Yapılan İşlem:</span> {service.workDone}
            </p>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
              Tamamlama Açıklaması / Son Durum Notu
            </label>
            <textarea
              value={completionNote}
              onChange={(e) => setCompletionNote(e.target.value)}
              placeholder="Örn: Cihaz tamir edildi, gerekli parçalar takıldı ve müşteriye çalışır durumda teslim edildi..."
              rows={3}
